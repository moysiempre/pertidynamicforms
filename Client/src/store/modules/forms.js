...
const mutations = {
  ...
  ADD_DETAIL_ITEM: (state, detalle) => {
    if (
      state.formHd &&
      state.formHd.formDetails &&
      state.formHd.formDetails.length
    ) {
...
const actions = {
  ...
  loadBaseDetails({ commit }) {
    axios
      .get('api-forms/basedetail')
      .then(response => {
  ...
  loadUniqOptions({ commit }) {
    axios
      .get('api-landingpage/options')
      .then(response => {
        commit('SET_OPTIONS', response.data)
      })
      .catch(console.error)
